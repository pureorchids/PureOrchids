...
--// GUI SETUP (ONYX MODERN + MOBILE SUPPORT)
...
local mobileScale = isMobile and 0.55 or 1 -- Scale factor for mobile
local mobileSidebarWidth = isMobile and 140 or 220
local mobileMainWidth = isMobile and 480 or 850
local mobileMainHeight = isMobile and 360 or 550
local mobileTextScale = isMobile and 0.85 or 1
local mobileButtonHeight = isMobile and 32 or 40
local mobileToggleHeight = isMobile and 28 or 35
local mobileTabHeight = isMobile and 36 or 45
local mobileAvatarSize = isMobile and 60 or 90
local mobileProfileHeight = isMobile and 110 or 160
...
main.Name = "MainNode"
main.Size = UDim2.new(0, mobileMainWidth, 0, mobileMainHeight)
main.Position = UDim2.new(0.5, -mobileMainWidth/2, 0.5, -mobileMainHeight/2)
...
sidebar.Size = UDim2.new(0, mobileSidebarWidth, 1, 0)
...
-- Avatar Profile Header (Mobile Responsive)
local profile = Instance.new("Frame", sidebar)
profile.Size = UDim2.new(1, 0, 0, mobileProfileHeight)
profile.BackgroundTransparency = 1

local avImg = Instance.new("ImageLabel", profile)
avImg.Size = UDim2.new(0, mobileAvatarSize, 0, mobileAvatarSize)
avImg.Position = UDim2.new(0.5, -mobileAvatarSize/2, 0, isMobile and 10 or 20)
avImg.BackgroundTransparency = 1
avImg.Image = UserAvatarId
local avCorner = Instance.new("UICorner", avImg); avCorner.CornerRadius = UDim.new(1, 0)
local avStroke = Instance.new("UIStroke", avImg); avStroke.Color = C_ACCENT; avStroke.Thickness = isMobile and 1.5 or 2

local welcome = Instance.new("TextLabel", profile)
welcome.Size = UDim2.new(1, 0, 0, 16); welcome.Position = UDim2.new(0, 0, 0, isMobile and 75 or 120)
welcome.BackgroundTransparency = 1; welcome.Text = isMobile and "" or "Welcome Back,"
welcome.Font = Enum.Font.GothamMedium; welcome.TextColor3 = C_SUBTEXT; welcome.TextSize = isMobile and 10 or 13

local uName = Instance.new("TextLabel", profile)
uName.Size = UDim2.new(1, 0, 0, 20); uName.Position = UDim2.new(0, 0, 0, isMobile and 75 or 138)
uName.BackgroundTransparency = 1; uName.Text = lp.Name
uName.Font = Enum.Font.GothamBold; uName.TextColor3 = C_TEXT; uName.TextSize = isMobile and 12 or 16
uName.TextTruncate = Enum.TextTruncate.AtEnd

-- Tab Container (Mobile Responsive)
local tabScroll = Instance.new("ScrollingFrame", sidebar)
tabScroll.Size = UDim2.new(1, 0, 1, -mobileProfileHeight - 10); tabScroll.Position = UDim2.new(0, 0, 0, mobileProfileHeight + 5)
tabScroll.BackgroundTransparency = 1; tabScroll.ScrollBarThickness = isMobile and 3 or 2
local tabLayout = Instance.new("UIListLayout", tabScroll); tabLayout.Padding = UDim.new(0, isMobile and 3 or 5); tabLayout.HorizontalAlignment = Enum.HorizontalAlignment.Center

-- Content Area (Mobile Responsive)
local content = Instance.new("Frame", main)
content.Name = "Content"
content.Size = UDim2.new(1, -mobileSidebarWidth, 1, 0)
content.Position = UDim2.new(0, mobileSidebarWidth, 0, 0)
content.BackgroundTransparency = 1

-- Top Bar (Close Button) - Mobile Responsive
local topBar = Instance.new("Frame", content)
topBar.Size = UDim2.new(1, 0, 0, isMobile and 30 or 40); topBar.BackgroundTransparency = 1

-- ** TITLE ADDITION ** (Mobile Responsive)
local titleLbl = Instance.new("TextLabel", topBar)
titleLbl.Size = UDim2.new(0, isMobile and 150 or 200, 1, 0)
titleLbl.Position = UDim2.new(0, isMobile and 8 or 20, 0, 0)
titleLbl.BackgroundTransparency = 1
titleLbl.Text = isMobile and "TaxHub V5" or "TaxHub V5 *Prison Life*"
titleLbl.Font = Enum.Font.GothamBlack
titleLbl.TextSize = isMobile and 14 or 24
titleLbl.TextColor3 = C_ACCENT -- Neon Mint match
titleLbl.TextXAlignment = Enum.TextXAlignment.Left

local closeBtn = Instance.new("TextButton", topBar)
closeBtn.Size = UDim2.new(0, isMobile and 30 or 40, 0, isMobile and 30 or 40); closeBtn.Position = UDim2.new(1, isMobile and -30 or -40, 0, 0); closeBtn.Text = "×"; closeBtn.Font = Enum.Font.GothamMedium; closeBtn.TextSize = isMobile and 18 or 24
...
pageContainer.Size = UDim2.new(1, isMobile and -16 or -40, 1, isMobile and -40 or -60); pageContainer.Position = UDim2.new(0, isMobile and 8 or 20, 0, isMobile and 35 or 50); pageContainer.BackgroundTransparency = 1
...
local function CreateTab(name, iconId)
    ...
    btn.Size = UDim2.new(0, mobileSidebarWidth - (isMobile and 10 or 30), 0, mobileTabHeight)
    btn.BackgroundColor3 = C_SIDE
    btn.Text = isMobile and ("  " .. name) or ("       " .. name)
    ...
    Instance.new("UICorner", btn).CornerRadius = UDim.new(0, isMobile and 6 or 8)
    
    local ind = Instance.new("Frame", btn); ind.Size = UDim2.new(0, isMobile and 3 or 4, 0, isMobile and 18 or 24); ind.Position = UDim2.new(0, 0, 0.5, isMobile and -9 or -12); ind.BackgroundColor3 = C_ACCENT; ind.BackgroundTransparency = 1
    Instance.new("UICorner", ind).CornerRadius = UDim.new(0, 2)
    
    if iconId and not isMobile then
    ...
    page.Size = UDim2.new(1, 0, 1, 0); page.BackgroundTransparency = 1; page.ScrollBarThickness = isMobile and 4 or 2; page.Visible = false
    local layout = Instance.new("UIListLayout", page); layout.Padding = UDim.new(0, isMobile and 6 or 10); layout.SortOrder = Enum.SortOrder.LayoutOrder
...
local function CreateSection(page, title, isDual) -- Added legacy param support just in case (Mobile Responsive)
    local s = Instance.new("Frame", page)
    s.Size = UDim2.new(1, isMobile and -4 or -10, 0, isMobile and 24 or 30) -- Height scales
    s.BackgroundColor3 = C_ELEM
    Instance.new("UICorner", s).CornerRadius = UDim.new(0, isMobile and 6 or 10)
    
    local lbl = Instance.new("TextLabel", s); lbl.Text = "  " .. title; lbl.Size=UDim2.new(1,0,0, isMobile and 22 or 30); lbl.BackgroundTransparency=1; lbl.TextColor3=C_SUBTEXT; lbl.Font=Enum.Font.GothamBold; lbl.TextSize= isMobile and 9 or 12; lbl.TextXAlignment=Enum.TextXAlignment.Left
    
    local cont = Instance.new("Frame", s); cont.Size=UDim2.new(1, isMobile and -10 or -20, 0, 0); cont.Position=UDim2.new(0, isMobile and 5 or 10, 0, isMobile and 26 or 35); cont.BackgroundTransparency=1
    local lay = Instance.new("UIListLayout", cont); lay.Padding=UDim.new(0, isMobile and 5 or 8); lay.SortOrder=Enum.SortOrder.LayoutOrder
    lay:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
        cont.Size = UDim2.new(1, isMobile and -10 or -20, 0, lay.AbsoluteContentSize.Y)
        s.Size = UDim2.new(1, isMobile and -4 or -10, 0, lay.AbsoluteContentSize.Y + (isMobile and 32 or 45))
    end)
    return cont
end
...
local function CreateButton(parent, text, color, cb)
    ...
    btn.Size = UDim2.new(1, 0, 0, mobileButtonHeight)
    btn.BackgroundColor3 = Color3.fromRGB(32, 32, 36)
    btn.Text = text
    btn.TextColor3 = color or C_TEXT
    btn.Font = Enum.Font.GothamBold; btn.TextSize = isMobile and 10 or 14
    btn.TextTruncate = Enum.TextTruncate.AtEnd
    Instance.new("UICorner", btn).CornerRadius = UDim.new(0, isMobile and 6 or 8)
...
local function CreateToggle(parent, text, default, cb)
    ...
    frame.Size = UDim2.new(1, 0, 0, mobileToggleHeight)
    frame.BackgroundColor3 = Color3.fromRGB(32, 32, 36)
    frame.Text = "  " .. text
    frame.TextColor3 = C_TEXT
    frame.Font = Enum.Font.GothamMedium; frame.TextSize = isMobile and 10 or 14; frame.TextXAlignment = Enum.TextXAlignment.Left
    frame.AutoButtonColor = false
    frame.TextTruncate = Enum.TextTruncate.AtEnd
    Instance.new("UICorner", frame).CornerRadius = UDim.new(0, isMobile and 6 or 8)
    
    local indSize = isMobile and 16 or 20
    local ind = Instance.new("Frame", frame)
    ind.Size = UDim2.new(0, indSize, 0, indSize); ind.Position = UDim2.new(1, -(indSize + 10), 0.5, -indSize/2)
...
local function CreateSlider(parent, text, min, max, default, cb)
    ...
    frame.Size = UDim2.new(1, 0, 0, isMobile and 38 or 50)
    frame.BackgroundTransparency = 1
    
    local lbl = Instance.new("TextLabel", frame); lbl.Size=UDim2.new(1,0,0, isMobile and 16 or 20); lbl.BackgroundTransparency=1; lbl.Text = text .. ": " .. default; lbl.TextColor3=C_TEXT; lbl.Font=Enum.Font.GothamMedium; lbl.TextSize= isMobile and 10 or 14; lbl.TextXAlignment=Enum.TextXAlignment.Left
    
    local barHeight = isMobile and 10 or 6 -- Taller bar for mobile touch
    local bar = Instance.new("Frame", frame); bar.Size=UDim2.new(1,0,0,barHeight); bar.Position=UDim2.new(0,0,0, isMobile and 22 or 30); bar.BackgroundColor3=Color3.fromRGB(20,20,25)
...
local function CreateLabel(parent, text, col) -- Added col to signature matching (Mobile Responsive)
    local l = Instance.new("TextLabel", parent)
    l.Size = UDim2.new(1, 0, 0, isMobile and 16 or 20); l.BackgroundTransparency=1; l.Text = text; l.TextColor3=col or C_SUBTEXT; l.Font=Enum.Font.Gotham; l.TextSize= isMobile and 10 or 13; l.TextXAlignment=Enum.TextXAlignment.Left
